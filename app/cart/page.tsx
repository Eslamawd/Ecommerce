  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-6xl p-6 md:p-10">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-3xl font-bold">{t("cart_title")}</h1>
      ...
      {cartQuery.isLoading ? (
        <p className="text-sm text-muted">{t("cart_loading")}</p>
      ) : null}
        ...
        <section className="space-y-4 rounded-2xl bg-card p-5 shadow-soft md:p-7">
          <div className="flex items-center justify-between">
            <p className="text-sm text-muted">
              {cart.items_count} {t("cart_items_count")}
            </p>
            <button
              type="button"
          ...
          {cart.items.length === 0 ? (
            <p className="text-sm text-muted">{t("cart_empty")}</p>
          ) : null}
            ...
            {cart.items.map((item) => (
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-muted">
                    {t("cart_subtotal")}: ${Number(item.subtotal).toFixed(2)}
                  </p>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    disabled={updateItemMutation.isPending || !item.product}
                  >
                    -
                  </button>
                  ...
                  <button
                    ...
                    disabled={removeItemMutation.isPending || !item.product}
                  >
                    {t("cart_remove")}
                  </button>
                </div>
          ...
          <div className="border-t border-slate-200 pt-4 dark:border-slate-700">
            <p className="text-lg font-semibold">
              {t("cart_total")}: ${Number(cart.total).toFixed(2)}
            </p>

            {cart.items.length > 0 ? (
              <Link
                href="/checkout"
                className="mt-4 inline-flex w-full items-center justify-center rounded-xl bg-accent px-4 py-2 font-semibold text-slate-950 transition hover:opacity-90"
              >
